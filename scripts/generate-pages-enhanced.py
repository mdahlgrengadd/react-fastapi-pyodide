...
class TypeScriptParser:
    ...
    def parse_api_functions(self) -> Dict[str, Any]:
        ...
        for match in re.finditer(function_pattern, content, re.DOTALL):
            ...
            logger.debug(f"Request config for {func_name}: {request_config[:100]}...")  # First 100 chars
        ...
        logger.info(f"Parsed {len(functions)} API functions from {self.sdk_file}")
        return functions
...
class EnhancedPageGenerator:
    ...
    def _group_functions_by_domain(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for func_name, func_info in self.api_functions.items():
            ...
            domain = self._extract_domain_from_function(func_name, func_info['url'])
    ...
    def _extract_domain_from_function(self, func_name: str, url: str) -> Optional[str]:
        ...
        patterns = [
            r'^(get|create|update|delete)([A-Z][a-z]+)s$',     # getUsers -> Users (plural)
            r'^(get|create|update|delete)([A-Z][a-z]+)$',      # getUser -> Users (singular, make plural)
        ]
    ...
    def _generate_domain_page(self, domain: str, functions: List[Dict[str, Any]]) -> Optional[str]:
        ...
        list_func = next((f for f in functions if f['method'] == 'GET' and not f['has_path_params']), None)
        detail_func = next((f for f in functions if f['method'] == 'GET' and f['has_path_params']), None)
    ...
    def _generate_page_component(
        ...
        # Generate imports
        imports = self._generate_imports(import_functions, import_types, detail_func is not None)
        
        # Generate main component
        main_component = self._generate_main_component(capitalized_domain, detail_func is not None)
        
        # Generate list component
        list_component = self._generate_list_component(domain, capitalized_domain, list_func, response_type)
        
        # Generate detail component (if needed)
        detail_component = ""
        if detail_func:
            detail_component = self._generate_detail_component(domain, capitalized_domain, detail_func, response_type)
...
"""
    ...
    def _generate_imports(self, functions: List[str], types: List[str], has_detail: bool) -> str:
        ...
        imports.append(f"import {{ {', '.join(router_imports)} }} from 'react-router-dom';")
        
        # API client imports
        if functions:
            imports.append(f"import {{ {', '.join(functions)} }} from '../client';")
        
        # Type imports
        if types:
            imports.append(f"import type {{ {', '.join(types)} }} from '../client';")
  ...
  return (
    ...
    def _update_page_index(self, domains: List[str]) -> None:
        ...
        for domain in sorted(domains):
            capitalized_domain = domain.capitalize()
            exports.append(f"export {{ {capitalized_domain}Page }} from './{capitalized_domain}Page';")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Generate React pages from TypeScript client")
    parser.add_argument(
        "--client-dir",
        type=Path,
