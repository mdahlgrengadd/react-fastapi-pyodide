...
class TypeScriptPageGenerator {
  constructor() {
    this.project = new Project({
      tsConfigFilePath: path.join(__dirname, '..', 'apps', 'frontend', 'tsconfig.json'),
    });
  }
  ...
  async generatePages() {
    ...
    const sdkFile = this.project.addSourceFileAtPath(path.join(CONFIG.clientDir, 'sdk.gen.ts'));
    const typesFile = this.project.addSourceFileAtPath(path.join(CONFIG.clientDir, 'types.gen.ts'));
    ...
    log(`📊 Found ${Object.keys(domains).length} domains: ${Object.keys(domains).join(', ')}`, colors.blue);
  ...
  generateListComponent(sourceFile, domain, functions, types) {
    const capitalizedDomain = domain.charAt(0).toUpperCase() + domain.slice(1);
    const listFunction = functions.find(f => f.name.startsWith('get') && f.name.includes(domain));
  ...
  generateDetailComponent(sourceFile, domain, functions, types) {
    const capitalizedDomain = domain.charAt(0).toUpperCase() + domain.slice(1);
    const detailFunction = functions.find(f => 
