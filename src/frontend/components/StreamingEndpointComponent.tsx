...
export const StreamingEndpointComponent: React.FC<StreamingEndpointComponentProps> = ({ 
  endpoint, 
  onError, 
  maxUpdates = 20 
}) => {
  ...
  const isStreamingEndpoint = endpoint.operationId.includes('async') || 
  ...
  const addUpdate = useCallback((updateData: Record<string, unknown>, step?: number, isComplete = false) => {
  ...
  const handleMonitoringStream = useCallback(async (
    body?: FormData,
    customQueryParams?: Record<string, string | number | boolean | undefined>
  ) => {
    const duration = Number(customQueryParams?.monitor_duration || 5);
    addUpdate({ status: 'monitoring', message: `Starting monitoring for ${duration} iterations...` });

    // Execute the full monitoring operation
    const result = await executeEndpoint(body, customQueryParams);
    const liveData = result?.content?.monitoring_session?.live_data as Array<Record<string, unknown>> || [];
  ...
  const handleSimulationStream = useCallback(async (
    body?: FormData,
    customQueryParams?: Record<string, string | number | boolean | undefined>
  ) => {
    const steps = Number(customQueryParams?.steps || 5);
    addUpdate({ status: 'simulation', message: `Starting simulation with ${steps} steps...` });

    const result = await executeEndpoint(body, customQueryParams);
    const simulationResults = result?.content?.simulation?.results as Array<Record<string, unknown>> || [];
  ...
  const startStreaming = useCallback(async (
    ...
    try {
      ...
      if (!isStreamingEndpoint) {
        ...
        const result = await executeEndpoint(body, customQueryParams);
        addUpdate({ status: 'completed', message: 'Operation completed' }, undefined, true);
        setFinalResult(result);
        return;
      }
      ...
      if (endpoint.operationId === 'async_monitor') {
        ...
        addUpdate({ status: 'starting', message: 'Starting async operation...' });
        const result = await executeEndpoint(body, customQueryParams);
  ...
  }, [endpoint, isStreamingEndpoint, executeEndpoint, addUpdate, onError, handleMonitoringStream, handleSimulationStream]);
    ...
    <div style={{ padding: "20px", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        {isStreamingEndpoint && (
          <div style={{ 
            ...
            🔄 <strong>Streaming Endpoint:</strong> This endpoint supports real-time updates
          </div>
        )}
      </div>
          ...
          >
            {updates.map((update, index) => (
              <div
                ...
                style={{
                  padding: "10px",
                  borderBottom: index < updates.length - 1 ? "1px solid #eee" : "none",
              ...
              >
                <div style={{ 
                  ...
                  <strong>
                    {update.step ? `Step ${update.step}` : `Update #${index + 1}`}
                    {update.isComplete && " ✅"}
                  </strong>
        ...
        <div>
          ...
          <div style={{
            ...
            <div style={{ 
              ...
              <span style={{
                backgroundColor: getStatusColor(finalResult.status_code || 200),
                color: "white",
                padding: "4px 8px",
                borderRadius: "4px",
                fontSize: "12px",
      ...
      {updates.length === 0 && !finalResult && !isStreaming && (
        <div style={{ 
          ...
          <p>Click "Start {isStreamingEndpoint ? 'Streaming' : 'Execution'}" to begin</p>
        </div>
      )}
    </div>
  );
