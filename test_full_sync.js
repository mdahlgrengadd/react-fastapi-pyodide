// Full Bidirectional Reactive Sync Test
// Run this in the browser console after Pyodide is initialized
...
pyodide_test_user_name = user.name
    ...
    try {
        ...
        if (createResponse.ok) {
            ...
            // Wait for SSE poke and pull
            console.log("⏳ Waiting for client to pull changes...");
            await new Promise(resolve => setTimeout(resolve, 2000));
            
            // Verify in Pyodide
            const pyodideUser = await pyodide.runPythonAsync(`
from app.domains.models import User
...
with runtime.open_session() as session:
    ...
    if user:
        ...
        pyodide_found_user = user.name
    else:
        print("❌ Not found in Pyodide")
        pyodide_found_user = None
            `);
            
            const foundName = pyodide.globals.get('pyodide_found_user');
            if (foundName) {
                console.log(`✅ Verified in Pyodide: ${foundName}`);
            } else {
                console.error("❌ User not found in Pyodide");
            }
        } else {
...
with runtime.open_session() as session:
    from sqlmodel import select
    user = session.exec(select(User).where(User.id == ${targetUser.id})).first()
    if user:
        old_name = user.name
        user.name = "${newName}"
        user.save()
