...
const DashboardsList: React.FC = () => {
  const {
    ...
    isLoading,
    error
  } = useQuery(['dashboards'], () => getDashboard());

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto p-8">        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Dashboards Management</h1>
            <p className="text-gray-600">Manage dashboards data and entries</p>
          </div>
          <Link
            to="/"
        ...
        {error ? (
          <div className="bg-white rounded-lg shadow-lg p-6">
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <p className="text-red-800 font-medium">Failed to load dashboards</p>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-medium text-gray-900">
                          {item.name || item.title || item.email || `Dashboards ${item.id || index + 1}`}
                        </h3>
                        {item.description && (
