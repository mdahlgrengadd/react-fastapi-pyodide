...
import { getAsyncDashboardStats } from '../client';
...
const DashboardList: React.FC = () => {
  const {
    ...
    isLoading,
    error
  } = useQuery(['dashboard'], () => getAsyncDashboardStats());

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto p-8">        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Dashboard Management</h1>
            <p className="text-gray-600">Manage dashboard data and entries</p>
          </div>
          <Link
            to="/"
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-medium text-gray-900">
                          {item.name || item.title || item.email || `Dashboard ${item.id || index + 1}`}
                        </h3>
                        {item.description && (
